...
#opt$rlog <- "output/Arabidopsis_thaliana/FEATURECOUNTS/TAIR10.gene.rlog.tsv"
#opt$organism <- "Arabidopsis_thaliana"
#opt$plots <- "output/Arabidopsis_thaliana/PCA"
#opt$term2gene <- "input/Arabidopsis_thaliana/GENOME/TERM2GENE.tsv"
#opt$term2name <- "input/Arabidopsis_thaliana/GENOME/TERM2NAME.tsv"
...
#ggsave(plot = ind, file = file.path(opt$plots, "ind.svg"))
...
#genes.ref <- sapply(sample.types, function(sample.type){
#  rownames(head(pca.var.details[order(pca.var.details[[sample.type]], decreasing=FALSE),], 1000))
#})

#genes.ref <- unlist(as.list(genes.ref))
#genes.ref <- unique(genes.ref)
...
for(sample.type in sample.types){
 ...
 # go.enrichment.plot <- pca_go(
  #  rownames(head(pca.var.details[order(pca.var.details[[sample.type]], decreasing=FALSE),], 200)),
#    genes.ref,
  #  sample.type,
 #   taxon.id = opt$taxon)
